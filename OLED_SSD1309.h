/*
	OLED_SSD1309 - An Hardware Accellerated and CPU optimized library for drive
	SOLOMON TECH SSD1309 OLED display via SPI serial interface.
    ...
    OLED_SSD1309 Library is free software: you can redistribute it and/or modify
    it under the terms of the GNU General Public License as published by
    the Free Software Foundation, either version 3 of the License, or
    (at your option) any later version.

    OLED_SSD1309 Library is distributed in the hope that it will be useful,
...
#ifndef _OLED_SSD1309H_
#define _OLED_SSD1309H_
	...
	#include "_settings/OLED_SSD1309_registers.h"
	#include "_settings/OLED_SSD1309_settings.h"
		...
		static SPISettings SSD1309_SPI;
	#endif

	enum OLEDSSD1309SCRLLTYPE {HR=0,HL=1,VR=2,VL=3 };
...
// the class
class OLED_SSD1309 : public Print {
 public:
	#if defined(__MK20DX128__) || defined(__MK20DX256__)
		OLED_SSD1309(uint8_t cspin,uint8_t dcpin,uint8_t rstpin=255,uint8_t mosi=11,uint8_t sclk=13);
	#elif defined(__MKL26Z64__)
		OLED_SSD1309(uint8_t cspin,uint8_t dcpin,uint8_t rstpin=255,uint8_t mosi=11,uint8_t sclk=13);
	#else
		OLED_SSD1309(uint8_t cspin,uint8_t dcpin,uint8_t rstpin=255);
	#endif
	uint8_t	getError();
	void 	begin(uint8_t switchvcc = _CMD_SWITCHCAPVCC);
	void 	invertDisplay(uint8_t i);
	void 	startScroll(enum OLEDSSD1309SCRLLTYPE type,uint8_t start, uint8_t stop);
 ...
 protected:
	...
	const common_font_t *_font;
		...
		__attribute__((always_inline)) {
			#if defined(SPI_HAS_TRANSACTION)
				SPI.beginTransaction(SSD1309_SPI);
			#endif
		}
		...
		void OLED_SSD1309::spiwrite(uint8_t c)
		__attribute__((always_inline)) {
			SPDR = c;
			while(!(SPSR & _BV(SPIF)));
		}
